---
title: "Orchestrator pack: state files for multi-agent coordination"
...
<Tabs>
  ...
  <Tab title="budget">
    Hard limits on resources: `max_seconds` (wall-clock time), `max_tokens` (LLM token budget), `max_turns` (reasoning turns), `max_depth` (delegation depth — 0 for workers), and `max_disk_mb`. Every agent has a budget. No infinite recursion. No unbounded token spend.
  </Tab>
